...
@section('TopSectionName', 'Manage Epe/Ikeja Executives')
{{-- Still serving as secretaries --}}
@section('style')
    <style>
            ...
            <div class="row">
                <div class="col-12">
                    <h2 class="contact-title">
                        Add Epe/Ikeja Executives Record
                    </h2>
                    <hr>
                ...
                <div class="col-md-12">
                    <form class="form-contact contact_form" action="{{ route('secretaries.addEntry') }}"
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="position" id="position" class="form-control" minlength="5"
                                        maxlength="150" autocomplete="on" required
                                        placeholder="Enter Position (ex: Evangelism Cordinator)">
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="url" name="instagram" id="instagram" class="form-control"
                                        placeholder="Enter Social link (ex: https://instagram ...)">
                                </div>
                            </div>
                        </div>
                    ...
                    @if ($secretariesAuth)
                        <p class="mt-3 float-right">
                            <strong>Last updated by:</strong> {{ $secretariesAuth->user->name ?? 'Unknown' }} at
                            {{ $secretariesAuth->updated_at->format('d, M, Y. h:i A') }}
                        </p>
                    @endif
                </div>
            </div>
                ...
                <div class="progress-table-wrap">
                    <div class="progress-table">
                        <div class="table-head">
                            ...
                            <div class="serial" style="width: 300px;">Social</div>
                            <div class="percentage">Actions</div>
                        </div>
                        @foreach ($secretaries as $record)
                            <div class="table-row">
                                ...
                                <div class="percentage">{{ $record->name }}</div>
                                <div class="visit">
                                    {{ \Illuminate\Support\Str::limit(strip_tags($record->position), 50, '...') }}
                                </div>
                                <div class="serial" style="width: 300px;">
                                    @if ($record->instagram)
                                        <a href="{{ $record->instagram }}" target="_blank" class="text-muted">View
                                            Link</a>
                                    @else
                                        N/A
                                    @endif
                                ...
                                <div class="percentage">
                                    <div class="btn-group btn-group-sm">
                                        <a href="{{ route('secretaries.edit', $record) }}" class="btn bg-info">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('secretaries.delete', $record->id) }}" method="POST"
